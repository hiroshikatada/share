...
const requiredResolvedColumns = ["TXN_TYPE", "TXN_COUNT"];
for (const columnName of requiredResolvedColumns) {
  const rows = result.exported_tables.output_lineages.filter((item) => item.output_column_name === columnName);
  if (rows.length === 0) throw new Error(`${columnName} was not exported`);
  if (!rows.some((row) => row.lineage_status === "RESOLVED")) {
    throw new Error(`${columnName} had no RESOLVED lineage`);
  }
}
console.log(JSON.stringify({ warning_count: result.analysis.warning_count, lineage_diagnostics: diagnostics.length, remaining: diagnostics.map((row) => row.message) }, null, 2));
