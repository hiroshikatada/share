...
CREATE TEMP FUNCTION parse_dependencies_with_field_paths(
  sql_text STRING,
  ...
  >>,
  field_path_metadata ARRAY<STRUCT<
    table_catalog STRING,
    table_schema STRING,
    table_name STRING,
    column_name STRING,
...
RETURNS ARRAY<STRUCT<
  ...
  source_top_level_column STRING,
...
function parseStarSelectItem(selectItem) {
  ...
  if (tokens[0].token === "*") {
    tokenIndex = 1;
  } else if (isIdentifierToken(tokens[0])) {
    /*
     * alias.* または alias.struct_field.* を認識する。
     */
    ...
    while (
      scanIndex + 1 < tokens.length &&
      tokens[scanIndex].token === "."
    ) {
      const nextToken = tokens[scanIndex + 1];

      if (nextToken.token === "*") {
        tokenIndex = scanIndex + 2;
        structPath = pathParts.length > 0
          ? pathParts.join(".")
          : null;
        break;
      }
      ...
      pathParts.push(nextToken.normalized_token);
      scanIndex += 2;
    }
...
function findQualifiedColumnReferences(
  ...
  /*
   * alias.column や alias.struct_field.child_field を抽出する。
   */
  for (
    let tokenIndex = 0;
    ...
    if (
      !expressionTokens[tokenIndex + 1] ||
      expressionTokens[tokenIndex + 1].token !== "." ||
      !isIdentifierToken(expressionTokens[tokenIndex + 2])
    ) {
      continue;
    }

    const pathParts = [];
    let scanIndex = tokenIndex + 1;
    let lastToken = aliasToken;

    while (
      scanIndex + 1 < expressionTokens.length &&
      expressionTokens[scanIndex].token === "." &&
      isIdentifierToken(expressionTokens[scanIndex + 1])
    ) {
      const pathToken = expressionTokens[scanIndex + 1];
      pathParts.push(pathToken.normalized_token);
      lastToken = pathToken;
      scanIndex += 2;
    }

    const sourceAlias = aliasToken.normalized_token;
    const source = sourceAliasMap.get(sourceAlias);

    references.push({
      source_alias: sourceAlias,
      source_name: source ? source.source_name : null,
      source_type: source ? source.source_type : null,
      source_column: pathParts.join("."),
          ...
          decorateDependency(
            {
              ...
              star_struct_path: starSpec.struct_path,
...
function buildFieldPathMetadataMap(metadataRows) {
  const metadataMap = new Map();

  for (const row of (metadataRows || [])) {
    if (!row || !row.table_name || !row.field_path) {
      continue;
    }
    ...
    const field = {
      column_name: (row.column_name || "").toUpperCase(),
      field_path: row.field_path.toUpperCase(),
    ...
    for (const key of keys) {
      ...
      metadataMap.get(key).push(field);
    }
  }
...
function findFieldMetadata(sourceName, fieldPath, fieldMetadataMap) {
  const normalizedSource = normalizeObjectName(sourceName);
  const normalizedPath = (fieldPath || "").toUpperCase();
  const candidateKeys = [normalizedSource];
  const parts = normalizedSource.split(".");

  if (parts.length >= 2) {
    candidateKeys.push(parts.slice(-2).join("."));
  }

  candidateKeys.push(parts[parts.length - 1]);

  for (const key of candidateKeys) {
    const fields = fieldMetadataMap.get(key) || [];
    const matched = fields.find(
      (field) => field.field_path === normalizedPath
    );
...
function findImmediateChildFields(
  sourceName,
  structPath,
  fieldMetadataMap
) {
  const normalizedSource = normalizeObjectName(sourceName);
  const normalizedPrefix = (structPath || "").toUpperCase();
  const candidateKeys = [normalizedSource];
  const objectParts = normalizedSource.split(".");

  if (objectParts.length >= 2) {
    candidateKeys.push(objectParts.slice(-2).join("."));
  }

  candidateKeys.push(objectParts[objectParts.length - 1]);
  ...
  for (const key of candidateKeys) {
    ...
    for (const field of fields) {
      if (prefix && field.field_path.indexOf(prefix) !== 0) {
        continue;
      }

      const remainder = prefix
        ? field.field_path.substring(prefix.length)
        : field.field_path;

      if (!remainder || remainder.indexOf(".") >= 0) {
        continue;
      }

      childMap.set(remainder, {
        column_name: remainder,
        field_path: prefix + remainder,
...
function expandDependencyToPhysical(
  ...
  if (
    ...
    const metadataColumns = rootDependency.star_struct_path
      ? findImmediateChildFields(
          currentDependency.source_name,
          rootDependency.star_struct_path,
          fieldMetadataMap
        )
      : findMetadataColumns(
          currentDependency.source_name,
    ...
    return metadataColumns
      ...
      .map(
        (column) => ({
          ...
          source_top_level_column:
            (column.field_path || column.column_name).split(".")[0],
          source_field_path:
            column.field_path || column.column_name,
          source_data_type:
          ...
          expansion_status: rootDependency.star_struct_path
            ? "STRUCT_STAR_METADATA_RESOLVED"
            : "STAR_METADATA_RESOLVED",
  ...
  if (currentDependency.source_type !== "CTE") {
    const fieldMetadata = findFieldMetadata(
      currentDependency.source_name,
      currentDependency.source_column,
      fieldMetadataMap
    );
    ...
    return [{
      ...
      source_top_level_column:
        currentDependency.source_column
          ? currentDependency.source_column.split(".")[0]
          : null,
      source_field_path:
        currentDependency.source_column,
      source_data_type:
        fieldMetadata ? fieldMetadata.data_type : null,
      lineage_path:
        appendLineagePath([
          ...pathParts,
          currentDependency.source_name &&
  ...
  const outputPathParts =
    (outputColumn || "").toUpperCase().split(".");

  const cteTopLevelColumn = outputPathParts[0];
  const nestedSuffix = outputPathParts.slice(1).join(".");
  ...
  for (const candidate of candidateDependencies) {
    const candidateWithNestedPath = nestedSuffix
      ? {
          ...candidate,
          source_column: candidate.source_column
            ? candidate.source_column + "." + nestedSuffix
            : nestedSuffix
        }
      : candidate;
...
function resolveAllDependenciesToPhysical(
  dependencies,
  columnMetadataMap,
  fieldMetadataMap
) {
...
return physicalDependencies.map(
  (dependency, index) => ({
    ...
    source_top_level_column:
      dependency.source_top_level_column ||
      (dependency.source_column
        ? dependency.source_column.split(".")[0]
        : null),
    source_field_path:
      dependency.source_field_path || dependency.source_column,
    source_data_type:
      dependency.source_data_type || null,
    lineage_path:
...
--
-- Replace the following identifiers before execution:
--   your_project       : project containing the target Views/tables
--   your_dataset       : dataset containing the target Views/tables
--   repository_project : project containing the lineage repository
--   repository_dataset : dataset containing the lineage repository
--
-- Graph direction:
--   source column -> destination View column / View object
-- =====================================================================
...
CREATE TABLE IF NOT EXISTS
  `repository_project.repository_dataset.lineage_node`
(
  ...
  object_name STRING,
  object_full_name STRING NOT NULL,
  column_name STRING,
  field_path STRING,
  is_physical BOOL,
...
CREATE TABLE IF NOT EXISTS
  `repository_project.repository_dataset.lineage_edge`
(
  edge_id STRING NOT NULL,
  source_node_id STRING NOT NULL,
  target_node_id STRING NOT NULL,

  destination_view_catalog STRING NOT NULL,
  destination_view_schema STRING NOT NULL,
  destination_view_name STRING NOT NULL,
  destination_view_full_name STRING NOT NULL,
...
CREATE TEMP FUNCTION qualify_object_name(
  object_name STRING,
...
AS (
  CASE
    ...
    -- Preserve non-object expressions such as UNNEST expressions.
    WHEN REGEXP_CONTAINS(object_name, r'[\(\)]') THEN object_name

    WHEN ARRAY_LENGTH(SPLIT(object_name, '.')) >= 3
      THEN object_name

    WHEN ARRAY_LENGTH(SPLIT(object_name, '.')) = 2
      THEN CONCAT(default_catalog, '.', object_name)
...
-- ---------------------------------------------------------------------
-- 3. Metadata arrays passed into the JavaScript UDF
--
-- Dataset-scoped INFORMATION_SCHEMA is used here. To analyze several
-- datasets, UNION the corresponding metadata into these temporary tables.
-- ---------------------------------------------------------------------

CREATE TEMP TABLE column_metadata AS
SELECT
  table_catalog,
  table_schema,
  table_name,
...
CREATE TEMP TABLE field_path_metadata AS
SELECT
  table_catalog,
  table_schema,
  table_name,
  ...
  data_type
FROM
  `your_project.your_dataset.INFORMATION_SCHEMA.COLUMN_FIELD_PATHS`;


CREATE TEMP TABLE view_inventory AS
SELECT
  table_catalog,
  table_schema,
  table_name,
...
-- ---------------------------------------------------------------------
-- 4. Parse every View definition
-- ---------------------------------------------------------------------

CREATE TEMP TABLE parsed_dependency AS
WITH metadata AS (
  SELECT
    ARRAY_AGG(
      STRUCT(
...
field_metadata AS (
  ...
  FROM field_path_metadata
)
SELECT
  views.table_catalog AS destination_view_catalog,
  views.table_schema AS destination_view_schema,
  views.table_name AS destination_view_name,
  views.view_full_name AS destination_view_full_name,
  dependency.*
FROM
  view_inventory AS views
CROSS JOIN metadata
CROSS JOIN field_metadata
CROSS JOIN UNNEST(
  parse_dependencies_with_field_paths(
...
-- ---------------------------------------------------------------------
-- 5. Normalize source and destination graph endpoints
--
-- Projection dependencies target a View column.
-- Filter/join/group-only dependencies target the View object pseudo-node.
-- ---------------------------------------------------------------------

CREATE TEMP TABLE graph_edge_stage AS
SELECT
  parsed.*,

  qualify_object_name(
    parsed.source_name,
    parsed.destination_view_catalog,
    parsed.destination_view_schema
  ) AS source_object_full_name,

  CASE
    WHEN parsed.output_column IS NOT NULL
      THEN parsed.output_column
    ELSE '__VIEW__'
  END AS target_field_path,

  CASE
    WHEN parsed.output_column IS NOT NULL
      THEN 'VIEW_COLUMN'
    ELSE 'VIEW_OBJECT'
  END AS target_node_type

FROM parsed_dependency AS parsed
WHERE
  parsed.source_name IS NOT NULL
  AND parsed.source_column IS NOT NULL;


-- ---------------------------------------------------------------------
-- 6. Classify source nodes
--
-- If the resolved source object appears in INFORMATION_SCHEMA.VIEWS,
-- it is a View column. Otherwise it is treated as a physical table column.
-- ---------------------------------------------------------------------

CREATE TEMP TABLE classified_edge_stage AS
SELECT
  stage.*,

  CASE
    WHEN source_view.view_full_name IS NOT NULL
      THEN 'VIEW_COLUMN'

    WHEN stage.source_type = 'UNNEST'
      THEN 'DERIVED_FIELD'

    ELSE 'TABLE_COLUMN'
  END AS source_node_type,

  source_view.view_full_name IS NULL
    AND stage.source_type != 'UNNEST'
    AS source_is_physical

FROM graph_edge_stage AS stage
LEFT JOIN view_inventory AS source_view
  ON UPPER(source_view.view_full_name)
   = UPPER(stage.source_object_full_name);


-- ---------------------------------------------------------------------
-- 7. Rebuild nodes and edges atomically for this dataset
-- ---------------------------------------------------------------------
...
DELETE FROM
  `repository_project.repository_dataset.lineage_edge`
WHERE
  destination_view_catalog = 'your_project'
  AND destination_view_schema = 'your_dataset';


CREATE TEMP TABLE lineage_node_stage AS

WITH source_nodes AS (
  SELECT DISTINCT
    node_id(
      source_node_type,
      source_object_full_name,
      source_column
    ) AS node_id,
    ...
    CASE
      WHEN ARRAY_LENGTH(SPLIT(source_object_full_name, '.')) >= 3
        THEN SPLIT(source_object_full_name, '.')[SAFE_OFFSET(0)]
      ELSE NULL
    END AS object_catalog,

    CASE
      WHEN ARRAY_LENGTH(SPLIT(source_object_full_name, '.')) >= 3
        THEN SPLIT(source_object_full_name, '.')[SAFE_OFFSET(1)]
      ELSE NULL
    END AS object_schema,

    CASE
      WHEN ARRAY_LENGTH(SPLIT(source_object_full_name, '.')) >= 3
        THEN SPLIT(source_object_full_name, '.')[SAFE_OFFSET(2)]
      ELSE source_object_full_name
    END AS object_name,

    source_object_full_name AS object_full_name,

    SPLIT(source_column, '.')[SAFE_OFFSET(0)]
      AS column_name,

    source_column AS field_path,

    source_is_physical AS is_physical

  FROM classified_edge_stage
),
...
target_nodes AS (
  SELECT DISTINCT
    node_id(
      target_node_type,
      destination_view_full_name,
      target_field_path
    ) AS node_id,

    target_node_type AS node_type,

    destination_view_catalog AS object_catalog,
    destination_view_schema AS object_schema,
    destination_view_name AS object_name,
    destination_view_full_name AS object_full_name,

    CASE
      WHEN target_field_path = '__VIEW__'
        THEN NULL
      ELSE SPLIT(target_field_path, '.')[SAFE_OFFSET(0)]
    END AS column_name,

    CASE
      WHEN target_field_path = '__VIEW__'
        THEN NULL
      ELSE target_field_path
    END AS field_path,

    FALSE AS is_physical
...
MERGE
  `repository_project.repository_dataset.lineage_node` AS target
USING
  lineage_node_stage AS source
ON
...
WHEN MATCHED THEN
  UPDATE SET
    ...
    object_catalog = source.object_catalog,
    object_schema = source.object_schema,
    object_name = source.object_name,
    object_full_name = source.object_full_name,
    column_name = source.column_name,
    field_path = source.field_path,
    is_physical = source.is_physical,
...
WHEN NOT MATCHED THEN
  ...
  (
    source.node_id,
    source.node_type,
    source.object_catalog,
    source.object_schema,
    source.object_name,
    source.object_full_name,
    source.column_name,
    source.field_path,
    source.is_physical,
    source.created_at
...
(
  ...
  destination_view_catalog,
  destination_view_schema,
...
SELECT
  TO_HEX(
    SHA256(
      CONCAT(
        ...
        COALESCE(query_name, ''),
        '|',
        CAST(COALESCE(start_token_seq, -1) AS STRING),
        '|',
        CAST(COALESCE(end_token_seq, -1) AS STRING)
      )
    )
  ) AS edge_id,
  ...
  source_name AS resolved_source_name,
  source_type AS resolved_source_type,
  source_column AS resolved_source_column,
...
-- 8. Impact search
--
-- Starting from a physical column, walk downstream:
-- physical column -> View column -> higher-level View column ...
--
-- Replace the three seed values.
-- =====================================================================
...
WITH RECURSIVE impact AS (
  SELECT
    ...
    CAST(NULL AS STRING) AS via_usage_type,
    CAST(NULL AS STRING) AS via_expression,
    CAST(NULL AS STRING) AS via_destination_view,

    [node.node_id] AS visited_node_ids,
  ...
  WHERE
    UPPER(node.object_full_name)
      = UPPER('your_project.your_dataset.source_table')
    AND
    UPPER(node.field_path)
      = UPPER('SOURCE_COLUMN')
  ...
  SELECT
    ...
    target.node_id,
    target.node_type,
    target.object_full_name,
    target.field_path,

    edge.usage_type AS via_usage_type,
    edge.expression AS via_expression,
    edge.destination_view_full_name
      AS via_destination_view,

    ARRAY_CONCAT(
      parent.visited_node_ids,
      [target.node_id]
    ) AS visited_node_ids,
    ...
    CONCAT(
      ...
      target.object_full_name,
      CASE
        WHEN target.field_path IS NULL
          THEN ''
        ELSE CONCAT('.', target.field_path)
      END
    ) AS impact_path

  FROM impact AS parent
  JOIN
    `repository_project.repository_dataset.lineage_edge`
      AS edge
    ON edge.source_node_id
     = parent.node_id
  ...
  WHERE
    NOT target.node_id
      IN UNNEST(parent.visited_node_ids)
)
...
-- =====================================================================
-- 9. Reverse lineage search
--
-- Starting from a View output column, walk upstream to its physical roots.
--
-- Replace the two target values.
-- =====================================================================
...
WITH RECURSIVE upstream AS (
  ...
  WHERE
    UPPER(node.object_full_name)
      = UPPER('your_project.your_dataset.target_view')
    AND
    UPPER(node.field_path)
      = UPPER('TARGET_COLUMN')
  ...
  SELECT
    ...
    CONCAT(
      ...
      source.object_full_name,
      CASE
        WHEN source.field_path IS NULL
          THEN ''
        ELSE CONCAT('.', source.field_path)
      END
    ) AS lineage_path

  FROM upstream AS parent
  JOIN
    `repository_project.repository_dataset.lineage_edge`
      AS edge
...
SELECT
  upstream.depth,
  upstream.node_type,
  upstream.object_full_name,
  upstream.field_path,
  upstream.via_usage_type,
  upstream.via_expression,
  upstream.lineage_path,
  node.is_physical
FROM upstream
JOIN
  `repository_project.repository_dataset.lineage_node` AS node
ON
  node.node_id = upstream.node_id
ORDER BY
  upstream.depth,
  upstream.object_full_name,
  upstream.field_path;
