...
-- ============================================================
-- Sample: COLUMNS + COLUMN_FIELD_PATHS metadata
-- ============================================================
...
WITH metadata AS (
  ...
  FROM
    `your_project`.`region-your_region`.INFORMATION_SCHEMA.COLUMNS
),
field_metadata AS (
  SELECT
    ARRAY_AGG(
  ...
  FROM
    `your_project`.`region-your_region`.INFORMATION_SCHEMA.COLUMN_FIELD_PATHS
)
SELECT
  dependency.*
FROM
...
SELECT
  ...
  c.address.* EXCEPT(internal_code),
  c.profile.contact.email AS email
FROM `your_project.source_dataset.customer` AS c
    """,
    metadata.columns,
...
field_metadata AS (
  ...
  FROM `your_project`.`region-your_region`.INFORMATION_SCHEMA.COLUMN_FIELD_PATHS
),
views AS (
  SELECT
    table_catalog,
