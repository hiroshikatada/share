...
class SelectParser {
   ...
   *
   * 現在の基本版ではDISTINCTとALLを対象とする。
   * AS STRUCTやAS VALUEは後続対応時にここへ追加できる。
   *
  ...
  #splitTopLevelByComma(tokens, itemDepth) {
    ...
    for (const token of tokens) {
      if (token.token === "," && token.paren_depth === itemDepth) {
        const trimmedItem = this.#trimCommentTokens(currentItem);
    ...
    const lastItem = this.#trimCommentTokens(currentItem);
  ...
  #findExplicitAlias(itemTokens) {
    ...
    for (let tokenIndex = itemTokens.length - 1; tokenIndex >= 0; tokenIndex--) {
      ...
      const aliasTokens = this.#trimCommentTokens(itemTokens.slice(tokenIndex + 1));
      ...
      const expressionTokens = this.#trimCommentTokens(
  ...
  #findImplicitAlias(itemTokens) {
    ...
    const expressionTokens = this.#trimCommentTokens(itemTokens.slice(0, aliasIndex));
