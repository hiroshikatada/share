...
--
-- INFORMATION_SCHEMA.COLUMNS is the source of truth for top-level columns.
-- INFORMATION_SCHEMA.COLUMN_FIELD_PATHS adds nested STRUCT/RECORD fields.
--
-- Example:
--   column_name = order_detail
--   field_path  = order_detail.product.product_id
--
-- Simple columns are not reloaded from COLUMN_FIELD_PATHS. This prevents the
-- top-level row from being registered twice while retaining both the parent
-- STRUCT column and all nested child fields.
--
-- The engine normalizes metadata internally, but uppercase values are retained
-- here to match the existing Repository/UDF convention.
-- --------------------------------------------------------------------------
SET physical_columns_json = (
  ...
  physical_columns_source AS (
    SELECT *
    FROM top_level_columns

    UNION ALL

    SELECT *
    FROM nested_field_paths
  )
  SELECT COALESCE(
    TO_JSON_STRING(
      ARRAY_AGG(
        STRUCT(
          ...
          UPPER(field_path) AS field_path,
