...
--     rows are restored from a temporary backup.
...
-- --------------------------------------------------------------------------
-- Cleanup for definitions that are no longer active.
-- --------------------------------------------------------------------------
DELETE FROM
...
-- --------------------------------------------------------------------------
-- Analyze active changed definitions.
-- --------------------------------------------------------------------------
FOR target IN (
  SELECT
    object_project,
...
DO
  BEGIN
    ...
    -- A partial result must not replace an existing valid dependency set.
    ...
    -- ----------------------------------------------------------------------
    -- Stage the full replacement dependency set before touching persistent
    -- dependencies.
    -- ----------------------------------------------------------------------
      ...
      SELECT
        ...
        LOWER(
          CASE ARRAY_LENGTH(SPLIT(path.physical_table_name, '.'))
            WHEN 3
              THEN SPLIT(path.physical_table_name, '.')[SAFE_OFFSET(0)]
        ...
        LOWER(
          CASE ARRAY_LENGTH(SPLIT(path.physical_table_name, '.'))
            WHEN 3
              THEN SPLIT(path.physical_table_name, '.')[SAFE_OFFSET(1)]
            WHEN 2
              THEN SPLIT(path.physical_table_name, '.')[SAFE_OFFSET(0)]
        ...
        LOWER(
          CASE ARRAY_LENGTH(SPLIT(path.physical_table_name, '.'))
            WHEN 3
              THEN SPLIT(path.physical_table_name, '.')[SAFE_OFFSET(2)]
            WHEN 2
              THEN SPLIT(path.physical_table_name, '.')[SAFE_OFFSET(1)]
    ...
    -- Stage diagnostics as well. Persistent diagnostics are replaced only
    -- after parsing and staging have both succeeded.
    ...
    -- Backup current rows so the exception handler can restore them if the
    -- DELETE + INSERT replacement fails.
  ...
  EXCEPTION WHEN ERROR THEN
    -- If persistent replacement began, remove any partial new rows and restore
    -- the previous published state from the temporary backup.
