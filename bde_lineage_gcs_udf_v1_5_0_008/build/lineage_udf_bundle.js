...
class BigQueryExporter {
  ...
  #exportDiagnostic(row) {
    return this.#withMetadata({
      ...
      diagnostic_json: this.runtimeCompact ? null : this.#toJson(row)
    });
  }
}
...
class SelectParser {
  ...
  #parseWildcard(expressionTokens) {
    ...
    const exclusions = [];
    let wildcardTokens = significantTokens;
    const exceptIndex = significantTokens.findIndex(
      (token) => token.normalized_token === "EXCEPT"
    );

    if (exceptIndex >= 0) {
      wildcardTokens = significantTokens.slice(0, exceptIndex);
      for (let index = exceptIndex + 1; index < significantTokens.length; index++) {
    ...
    if (wildcardTokens.length === 1 && wildcardTokens[0].token === "*") {
      return { wildcard_type: "ALL", wildcard_qualifier: null, wildcard_exclusions: exclusions };
    }
    ...
    return { wildcard_type: null, wildcard_qualifier: null, wildcard_exclusions: [] };
  }
    ...
    for (const outputColumn of context.output_column_resolution.output_columns) {
      ...
      for (const [expandedIndex, physicalColumn] of visibleColumns.entries()) {
        expansions.push({
          ...
          wildcard_expression: outputColumn.wildcard_qualifier
